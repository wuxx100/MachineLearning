...
if __name__ == '__main__':
    ...
    for day in range(2,3):
        ...
        for prnNum in range(1,33):
            path = 'navData/'+str(year)+'/'+str(day)+'/findIODC/prn'+str(prnNum)+'.csv'
            ...
            for i in range(1):
                ...
                for k, color in zip(yUnique, clrs):
                    ...
                    for i in np.unique(featurePrime[curIdx,33]):
                        data[data[33]==i].to_csv('navData/'+str(year)+'/'+str(day)+'/findIODC/cleanedData/prn_'+str(prnNum)+'_IODC_'+str(int(i))+'_.csv',header=None,index=True)
